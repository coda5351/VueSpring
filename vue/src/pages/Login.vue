...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Navigate to previous page or default to account profile page
    const redirectPath = sessionStorage.getItem('redirectAfterLogin')
    if (redirectPath) {
      sessionStorage.removeItem('redirectAfterLogin')
    }

    // Helper to make friendly board URL
    function toUrlFriendly(str: string): string {
      return encodeURIComponent(str.trim().toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9\-]/g, ''));
    }

    // If the user is a USER or CUSTOMER, take them to the first board on their account (if any)
    const role = data.user?.role || store.getters.user?.role
    const accountId = data.user?.account?.id || store.getters.user?.account?.id

    if (role === 'USER' || role === 'CUSTOMER') {
      // Try to fetch boards for the account and redirect to the first one if available
      try {
        if (accountId) {
          api.get(`/kanban/boards?accountId=${accountId}`)
            .then(res => res.ok ? res.json() : Promise.resolve([]))
            .then((boards: any[]) => {
              const first = Array.isArray(boards) && boards.length > 0 ? boards[0] : null
              setTimeout(() => {
                if (first && first.id) {
                  router.push(`/b/${first.id}/${toUrlFriendly(first.name || '')}`)
                } else {
          ...
          // return early so the generic fallback below does not also run
          return
        }
      } catch (e) {
        // Ignore and fall back to default redirect below
        console.warn('Failed to fetch boards for redirect', e)
      }
    }
